...
export default class ObracunObjekta {
                ...
                <table class="table table-dark w-100" id="popisArtikalaTable" data-order='[[ 1, "asc" ]]'>
                      <thead class="thead-dark">
                          <tr>
                              ...
                              <th scope="col" class="text-right align-middle" data-class-name="text-right align-middle" data-sortable="true">Iznos pojedinačnih ukupno [kn]</th>
                          </tr>
                      </thead>
                      <tbody></tbody>
                      <tfoot>
              ...
              <div id="print-receipt">
                ...
                <div class="table-responsive pt-3">
                  <table class="table table-dark w-100" id="dodatneInfoTable">
                        <thead class="thead-dark">
                            <tr>
                                <th scope="col" class="align-middle text-left" data-class-name="align-middle">Dodatne informacije <button type="button" class="btn btn-md btn-dark-f rounded-0 d-print-none" id="print-dodatne-info"><span class="material-icons md-18 lh-0">print</span> Ispiši dodatne informacije</button> </th>
                            </tr>
                        </thead>
                        <tbody>
                          <tr>
  ...
  async dohvatiPodatke(datumVrijemeOd, datumVrijemeDo) {
    ...
    // TODO: upisati adresu za dohvat obracuna objekta
    const ajaxURL = ``;
    ...
    const {
      ...
      VrijemeZadnjegRacuna} = data;
    dodatneInfo.innerHTML = `
    <tr>
      <td class="align-middle">Ustanova: ${UstanovaNaziv}, restoran: ${RestoranNaziv}</td>
    </tr>
    <tr>
      <td class="align-middle">Ukupni iznos računa (sa PDV-om i bez subvencije): ${RacuniUkupno} kn</td>
    </tr>
    <tr>
      <td class="align-middle">Ukupni iznos subvencije: ${SubvencijaUkupno} kn</td>
    </tr>
    <tr>
      <td class="align-middle">Ukupni iznos poreza: ${PorezUkupno} kn</td>
    </tr>
    <tr>
      <td class="align-middle">Ukupni broj računa: ${BrojRacuna + BrojStorniranihRacuna}, od toga broj izdanih: ${BrojRacuna} i broj storniranih: ${BrojStorniranihRacuna}</td>
    </tr>
    <tr>
  ...
  async initDataTable() {
    ...
    $('#popisArtikalaTable').DataTable( {
        ...
        'buttons': [
          {
            ...
            text: '<span class="material-icons md-18 lh-0">file_present</span> Izvoz tablice u excel (.xslx)',
          ...
          {
            extend: 'print',
            text: '<span class="material-icons md-18 lh-0">print</span> Ispis obračuna',
            title: 'Obračun objekta',
            messageTop: `
      ...
      'footerCallback': function( row, data, start, end, display ) {
        ...
        document.querySelector('#popisArtikalaTable > tfoot td:nth-of-type(3)').innerText = `${kolPo1} (${kolPo2})`;
        document.querySelector('#popisArtikalaTable > tfoot td:nth-of-type(4)').innerText = `${kolMe1} (${kolMe2})`;
        document.querySelector('#popisArtikalaTable > tfoot td:nth-of-type(5)').innerText = `${kolUk1} (${kolUk2})`;
        document.querySelector('#popisArtikalaTable > tfoot td:nth-of-type(6)').innerText = `${iznosPo1.toFixed(2)} (${iznosPo2.toFixed(2)})`;
      },
      'initComplete': () => {
        const input = `
        ...
        <div class="input-group input-group-lg d-inline-flex w-50 pr-4">
            <input type="datetime-local" id="datumVrijemeOd" name="datumVrijemeOd" class="form-control input-dark-f rounded-0" value="${datumi(-1)}T00:00" min="${datumi(-31)}T00:00" max="${datumi(0)}T24:00">
            <input type="datetime-local" id="datumVrijemeDo" name="datumVrijemeDo" class="form-control input-dark-f rounded-0" value="${datumi(0)}T00:00" min="${datumi(-31)}T00:00" max="${datumi(0)}T24:00">
        </div>
        `;
