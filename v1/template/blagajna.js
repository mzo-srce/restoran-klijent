...
import {appStatus, doAjax, statusBlagajne, to, ugasiTImere} from '../../Shared/js/asyncAjax';
...
export default class Blagajna {
  constructor(server, windowLocation, kontrolniBrojDjelatnika, obveznikPDV) {
    ...
    this.naplata = new Naplata(this.server, this.windowLocation, this.kontrolniBrojDjelatnika, this.obveznikPDV);
    this.kosarica = new Kosarica(this.obveznikPDV);
  }
      ...
      <div class="col-md-10 offset-md-1 col-lg-6 offset-lg-3 no-blur">
        <div class="card z-depth-4 rounded-0 my-5">
          <div class="card-body rounded-0">
            <p class="font-italic text-right">Status: <strong class="${window.navigator.onLine ? 'text-success' : 'text-danger'} statusBlagajne">${window.navigator.onLine ? 'online' : 'offline'}</strong> ${this.obveznikPDV ? `| Nefiskaliziranih računa: <span class="nefisrac">0</span>` : ''} | <span class="clock">00.00.0000. 00:00</span></p>
              <form>
                  ...
                  <div class="input-group input-group-lg mb-4">
                      ...
                      <div class="input-group-append">
                          <button class="btn btn-light-f m-0 px-3 z-depth-0 rounded-0 waves-effect" id="ponistiBrKartice" type="button">
                              <span class="material-icons md-18 lh-0">
                                  loop
                              </span>
                          </button>
                      </div>
                  ...
                  <div class="btn-group btn-group-toggle w-100 z-depth-0 rounded-0" data-toggle="buttons">
                      <label class="btn btn-lg btn-light-f form-check-label font-weight-bold m-0 z-depth-0 rounded-0 waves-effect active">
                          <input class="form-check-input" type="radio" name="manualAuto" value="1" autocomplete="off" checked><span class="material-icons md-18 lh-0">
                              credit_card
                      ...
                      <label class="btn btn-lg btn-light-f form-check-label font-weight-bold m-0 z-depth-0 rounded-0 waves-effect">
                          <input class="form-check-input" type="radio" name="manualAuto" value="2" autocomplete="off"><span class="material-icons md-18 lh-0">
                              keyboard
                          </span> Ručni unos
                      </label>
                  </div>
            ...
            <div class="row mt-2px pb-3">
                <div class="col">
                  <div class="card z-depth-0 rounded-0 border-light-f">
                    <div class="card-body font-italic text-right">
                      Status: <strong class="${window.navigator.onLine ? 'text-success' : 'text-danger'} statusBlagajne">${window.navigator.onLine ? 'online' : 'offline'}</strong>                      
                      ${this.obveznikPDV ? `| Nefiskaliziranih računa: <span class="nefisrac">0</span>` : ''} 
  ...
  async showStudentMenu(student, menu) {
    ...
    for (const i of menu) {
      ...
      const dostupno = () => {
        return `
                <div class="menu-item px-2 ${PotrosenoDanas >= GrupaArtiklaMaxKolicina ? 'iskoristenArtikl' : `${DisplayGrupaNaziv.toLocaleLowerCase()}`}" data-boja="${PotrosenoDanas >= GrupaArtiklaMaxKolicina ? '' : `${DisplayGrupaNaziv.toLocaleLowerCase()}`}" data-postoparticipacije="${PostoParticipacije}" data-subvencija="${Subvencija}" data-stopaporeza="${StopaPoreza}" data-pdviznos="${PDVIznos}">
                    <div class="float-left textID">ID: ${Id}</div>
                    <div class="float-right">Potrošeno: <span class="potrosenoDanas" data-potrosenodanas="${PotrosenoDanas}">${PotrosenoDanas}/${GrupaArtiklaMaxKolicina}</span></div>
                    <br/>
                    <div class="textNaziv text-center ${(IsMenu ? 'pt-2' : 'pt-2 pb-3')}">${truncate(Naziv)}</div>
                    <div class="textSastavnice font-italic text-center ${(IsMenu ? 'pb-3' : '')}">${(IsMenu ? `(${truncate(ArtikliMenuPrikaz)})` : '')}</div>
                    ...
                    <div class="posCijenaBottom px-3 pb-2 ${PotrosenoDanas >= GrupaArtiklaMaxKolicina ? 'd-none' : ''}">
      ...
      menuItems += `
                      <div class="col-6 col-md-4 col-lg-3 h-100 menuArtikl p-0" data-displaygrupaid="${DisplayGrupaId}" data-grupaartiklaid="${GrupaArtiklaId}" data-ismenu="${IsMenu}" data-menutype="${IsMenu && Naziv.includes('veget') ? 'menu-vege' : 'menu'}">
                          ${dostupno()}
                      </div>
                  `;
    }
        ...
        <div class="card-body">
          ...
          <img id="slikaStudenta" src="${(Slika === '' || Slika === null) ? `${this.windowLocation.replace(/\/?$/, '/')}dist/img/placeholder260x260.png` : `data:image/png;base64, ${Slika}`}" alt="slika studenta" class="img-thumbnail mr-3 img-fluid">
              <div class="media-body">
                  ...
                  <p class="p-0 m-0">Subvencija: ${SubvencijaIznosDodijeljeno} kn</p>
                  ...
                  <p class="p-0 m-0">Raspoloživ dnevni iznos: ${SubvencijaDnevniIznosRaspoloziv} kn ${SubvencijaDnevniIznosRaspoloziv < 0 ? '(iznos oduzet od idućeg dana)' : ''}</p>
      ...
      <div class="card rounded-0">
          <div class="card-body p-0 rounded-0">
              ...
              <div class="input-group input-group-lg">
                  <input type="text" id="pretraziArtikle" name="pretraziArtikle" class="form-control rounded-0 input-dark-f" placeholder="Pretražite dostupne artikle iz ponude po ID oznaci ili nazivu">
                  <div class="input-group-append">
    ...
    <div class="card z-depth-0 rounded-0">
      <div class="card-body pt-0 border-light-left-f border-light-right-f" id="print-offline-receipt">
        ...
        <div class="table-responsive">
            <table class="table table-borderd w-100 mb-0" id="kosaricaTable">
                ...
                <tfoot>
                  <tr>
                      <td colspan="4" class="text-right align-middle">Ukupno: 0.00 kn</td>
                  </tr>
                  <tr>
                      <td colspan="4" class="text-right align-middle">Ukupni iznos subvencije: 0.00 kn</td>
                  </tr>
                  <tr>
                      <td colspan="4" class="text-right align-middle h5">Ukupno za platiti: 0.00 kn</td>
                  </tr>
                </tfoot>
            </table>
        </div>
          ...
          <div class="d-none d-print-block pb-2">
            ...
            <div class="table-responsive">
                <table class="table table-borderd w-100 mb-0" id="printObracunPorezaTable">
                    ...
                    <tfoot>
                      <tr>
                        <td colspan="4" class="text-right align-middle">Ukupni iznos poreza: 0.00 kn</td>
                      </tr>
                    </tfoot>
                </table>
            </div>
        ...
        <div class="d-none d-print-block">
          <p class="p-0 m-0">
            Datum i vrijeme: ${(new Date()).toLocaleString('de-AT')}<br/>
      ...
      <div class="row" id="kosaricaButtonsRow">
        <div class="col">
          <div class="btn-group w-100 z-depth-0 rounded-0" role="group" aria-label="poništi i naplati košaricu">
            <button type="button" class="btn btn-lg btn-light-f m-0 z-depth-0 rounded-0" id="ponistiBrKarticeKosarica"><span class="material-icons lh-0">loop</span> Poništi</button>
            <button type="button" id="naplatiKosaricuBtn" class="btn btn-lg btn-light-f active m-0 z-depth-0 rounded-0"><span class="material-icons lh-0">library_add_check</span> Naplati</button>
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col">
          <p class="text-center py-1 m-0"><small>Izračun na ekranu može odstupati jednu lipu u odnosu na stvarni račun!</small></p>
        </div>
      </div>
    </div>
  `;
