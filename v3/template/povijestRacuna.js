...
import {appStatus, datumi, divStatusCard, doAjax, dtOptions, hideStatusCard, to, ugasiTImere, toEuro, toHrk, zaokruziNaDvije} from '../../Shared/js/asyncAjax';
      ...
      ${divStatusCard()}
        <div class="card bg-dark z-depth-0 rounded-0 d-none content-card">
            <div class="card-body p-0 rounded-0">
              <p class="text-white pt-4 pl-3">
                <strong>Ukupni iznos subvencije (offline) - subvencija koja se zaračunala na blagajni u trenutku izdavanja offline računa</strong>
                <br/>
                <strong>Ukupno za platiti (offline) - iznos koji je student platio u trenutku nastanka offline računa</strong>
              </p>
      ...
      'columns': [
        ...
        {'data': 'DatumVrijemeRacuna', 'searchable': false},
        {
          ...
          'render': ({UkupniIznosSubvencije, UkupniIznosZaPlatiti}) => {
            return `${zaokruziNaDvije(Number(UkupniIznosSubvencije) + Number(UkupniIznosZaPlatiti))} €`;
          },
        },
        {'data': 'UkupniIznosSubvencije', 'searchable': false, 'render': (data) => {
          return `${data} €`;
        },
        },
        {
          'data': null,
          'searchable': false,
          'render': ({UkupniIznosZaPlatiti}) => {
            return `${UkupniIznosZaPlatiti} €`;
          },
        },
        {
          'data': null,
