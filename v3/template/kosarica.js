...
export default class Kosarica {
  constructor(obveznikPDV, obveznikFiskalizacije) {
  ...
  dodaj(e) {
    ...
    if (el.querySelector('.potrosenoDanas')) {
      ...
      if (parseInt(PotrosenoDanas, 10) < parseInt(GrupaArtiklaMaxKolicina, 10)) {
        ...
        // const DisplayGrupaId = el.dataset.displaygrupaid;
  ...
  static povecajKosaricu({Id, Naziv, Cijena, GrupaArtiklaId, postoparticipacije, subvencija, stopaporeza, pdviznos, obveznikPDV}) {
    ...
    <tr>
        <td data-tdid="${Id}" data-grupaartiklaid="${GrupaArtiklaId}" class="text-left align-middle">${Naziv} (${postoparticipacije}%)<br/>(1 = ${Cijena.toFixed(2)} €)</td>
  ...
  static kosaricaUkupno(dnevni, dvostrukiDnevni, razinaPrava, obveznikPDV) {
    ...
    const _dnevni = parseFloat(dnevni);
    const _dvostrukiDnevni = parseFloat(dvostrukiDnevni) < 0 ? 0.00 : parseFloat(dvostrukiDnevni);
    ...
    const tableKosaricaRow =`
    <tr>
      <td colspan="4" class="text-right align-middle">Ukupno: ${zaokruziNaDvije(cijenaUkupno)} €</td>   
    </tr>
    <tr>
      <td colspan="4" class="text-right align-middle">Ukupni iznos subvencije: ${subvencijaUkupno > _dvostrukiDnevni ? `${zaokruziNaDvije(_dvostrukiDnevni)} €` : `${zaokruziNaDvije(subvencijaUkupno)} €`}</td>
    </tr>
    <tr>
      <td colspan="4" class="text-right align-middle h5">Ukupno za platiti: ${subvencijaUkupno > _dvostrukiDnevni ? `${zaokruziNaDvije(cijenaUkupno - _dvostrukiDnevni)} €` : `${zaokruziNaDvije(cijenaUkupno - subvencijaUkupno)} €`}</td>
    </tr>`;
    ...
    if (obveznikPDV) {
      ...
      const tableObracunRow = `
      <tr>
        <td colspan="4" class="text-right align-middle">Ukupni iznos poreza: ${zaokruziNaDvije(porezUkupno)} €</td>
      </tr>
      `;
