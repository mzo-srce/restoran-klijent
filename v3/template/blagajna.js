...
import {appStatus, doAjax, statusBlagajne, to, ugasiTImere, toEuro, toHrk} from '../../Shared/js/asyncAjax';
...
export default class Blagajna {
            ...
            <p class="font-italic text-right">
              ...
              <form>
                  ...
                  <div class="btn-group btn-group-toggle w-100 z-depth-0 rounded-0" data-toggle="buttons">
                      ...
                      <label class="btn btn-lg btn-light-f form-check-label font-weight-bold m-0 z-depth-0 rounded-0 waves-effect">
                          <input class="form-check-input" type="radio" name="manualAuto" value="3" autocomplete="off"><span class="material-icons md-18 ">
                              qr_code
                          </span> QR kod
                      </label>
                  </div>
                  ...
                  <div class="custom-control custom-checkbox pt-3">
                    <input type="checkbox" class="custom-control-input form-control" name="promjenaSmjeraCitanja" id="promjenaSmjeraCitanja" ${localStorage.promjenaSmjeraCitanja === 'true' ? 'checked' : ''}>
                    <label class="custom-control-label" for="promjenaSmjeraCitanja">
                        Ukoliko promjenu smjera čitanja broja iskaznice ne možete napraviti unutar službene aplikacije od čitača iskaznica, označite ovo polje.
                        Obratite pažnju na podatke dohvaćenog studenta jer promjenom smjera čitanja moguć je dohvat dva različita studenta!
                    </label>
                </div>
              </form>
          </div>
  ...
  async showStudentMenu(student, menu) {
        ...
        return `
                <div class="menu-item px-2 ${PotrosenoDanas >= GrupaArtiklaMaxKolicina ? 'iskoristenArtikl' : `${DisplayGrupaNaziv.toLocaleLowerCase()}`}" data-boja="${PotrosenoDanas >= GrupaArtiklaMaxKolicina ? '' : `${DisplayGrupaNaziv.toLocaleLowerCase()}`}" data-postoparticipacije="${PostoParticipacije}" data-subvencija="${Subvencija}" data-stopaporeza="${StopaPoreza}" data-pdviznos="${PDVIznos}">
                    ...
                    <div class="posCijenaBottom px-3 pb-2 d-none">
                      <div class="textCijena text-right"><small>cijena: ${Cijena} €</small></div>
                      <div class="textCijenaSaSubvencijom text-right"><small>cijena uz iskaznicu: ${ZaPlatiti} €</small></div>
                    </div>
                </div>`;
      };
        ...
        <div class="card-body">
          ...
          <img id="slikaStudenta" src="${(Slika === '' || Slika === null) ? `${this.windowLocation.replace(/\/?$/, '/')}dist/img/placeholder260x260.png` : `data:image/png;base64, ${Slika}`}" alt="slika studenta" class="img-thumbnail mr-3 img-fluid">
              <div class="media-body">
                  ...
                  <p class="p-0 m-0">Subvencija: ${SubvencijaIznosDodijeljeno} €</p>
                  ...
                  <p class="p-0 m-0">Raspoloživ dnevni iznos: ${SubvencijaDnevniIznosRaspoloziv} € ${SubvencijaDnevniIznosRaspoloziv < 0 ? '(iznos oduzet od idućeg dana)' : ''}</p>
    ...
    <div class="card z-depth-0 rounded-0">
            ...
            <table class="table table-borderd w-100 mb-0" id="kosaricaTable">
                ...
                <thead>
                    <tr>
                        ...
                        <th scope="col" class="text-right align-bottom text-nowrap pb-0">Cijena [€]</th>
                ...
                <tfoot>
                  <tr>
                      <td colspan="4" class="text-right align-middle">Ukupno: 0.00 €</td>
                  </tr>
                  <tr>
                      <td colspan="4" class="text-right align-middle">Ukupni iznos subvencije: 0.00 €</td>
                  </tr>
                  <tr>
                      <td colspan="4" class="text-right align-middle h5">Ukupno za platiti: 0.00 €</td>
                  </tr>
                </tfoot>
            </table>
        </div>
                ...
                <table class="table table-borderd w-100 mb-0" id="printObracunPorezaTable">
                    ...
                    <thead>
                        <tr>
                            ...
                            <td scope="col" class="text-right align-bottom pb-0">Osnovica [€]</td>
                            <td scope="col" class="text-right align-bottom pb-0">Iznos [€]</td>
                        </tr>
                    </thead>
                    <tbody>
                    </tbody>
                    <tfoot>
                      <tr>
                        <td colspan="4" class="text-right align-middle">Ukupni iznos poreza: 0.00 €</td>
                      </tr>
                    </tfoot>
                </table>
            </div>
      ...
      <div class="row">
        <div class="col">
          <p class="text-center py-1 m-0"><small>Izračun na ekranu može odstupati 0.01 € u odnosu na stvarni račun!</small></p>
        </div>
      </div>
    </div>
  `;
  ...
  async coreEvents() {
    ...
    $('input[type=\'radio\'][name=\'manualAuto\']').on('change', async () => {
      ...
      if (parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 1 || parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 3) {
        document.getElementById('unosBrKartice').disabled = true;
      } else {
        document.getElementById('unosBrKartice').disabled = false;
        document.getElementById('unosBrKartice').focus();
      }
    });
    ...
    $('#unosBrKartice').on('input', async (e) => {
      ...
      if (e.currentTarget.value.length === 14 && parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 1 ||
       e.currentTarget.value.length === 8 && parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 1 ||
       e.currentTarget.value.length === 19 && parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 2 ||
       e.currentTarget.value.length > 20 && parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 3) {
        await statusBlagajne(window.navigator.onLine);
        if (parseInt($('input[type=\'radio\'][name=\'manualAuto\']:checked').val(), 10) === 1 && document.getElementById('promjenaSmjeraCitanja').checked) e.currentTarget.value = e.currentTarget.value.match(/.{1,2}/g).reverse().join('');
    ...
    $('#unosKarticeCard').on('change', '#promjenaSmjeraCitanja', async () => {
      localStorage.promjenaSmjeraCitanja = localStorage.promjenaSmjeraCitanja === 'true' ? false : true;
    });
