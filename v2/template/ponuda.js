  ...
  async initDataTable() {
    ...
    $('#popisArtikalaTable').DataTable( {
      ...
      'columns': [
        ...
        {'data': 'Subvencija', 'searchable': false, 'render': (data) => `${toEuro.num(data)} € <hr class="m-0 p-0"/> ${data} Kn`},
        {'data': 'PostoParticipacije', 'searchable': false},
        {'data': 'Cijena', 'searchable': false, 'render': (data) => `${toEuro.num(data)} € <hr class="m-0 p-0"/> ${data} Kn`},
